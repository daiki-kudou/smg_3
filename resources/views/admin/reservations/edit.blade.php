            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>
                            {{ Form::radio('eat_in_prepare', 1, !empty($reservation['eat_in_prepare']) ? ($reservation['eat_in_prepare'] == 1 ? true : false) : true, ['id' => 'eat_in_prepare', $reservation['eat_in'] != 1 ? 'disabled' : '']) }}
            ...
            <tr>
                <td colspan="2">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <p>
                            <a class="more_btn user_link" target="_blank" rel="noopener" href="{{ url('/admin/clients/' . $reservation['user_id']) }}">顧客詳細</a>
                        </p>
                    </div>
                </td>
            </tr>
                ...
                <tbody class="venue_result">
                    <tr>
                        ...
                        <td colspan="1">
                            ...
                            @foreach (collect($reservation['bills'][0]['breakdowns'])->where('unit_type', 1) as $key => $value)
                                @if (strpos($value['unit_item'], '割引料金') !== false)
                                    {{ Form::text('venue_price', ((int) $reservation['bills'][0]['venue_price']) + ((int) -$value['unit_cost']), [
                                        'class' => 'form-control
                                                      col-xs-3',
                                        'readonly',
                                    ]) }}
                            ...
                            @elseif($loop->last)
                                {{ Form::text('venue_price', (int) $reservation['bills'][0]['venue_price'], [
                                    'class' => 'form-control
                                                  col-xs-3',
                                    'readonly',
                                ]) }}
                ...
                @foreach (collect($reservation['bills'][0]['breakdowns'])->where('unit_type', 2) as $key => $value)
                    @if (strpos($value['unit_item'], '割引料金') === false)
                        <tr>
                            ...
                            <td>
                                {{ Form::text('equipment_breakdown_subtotal[]', $value['unit_subtotal'], ['class' => 'form-control', 'readonly']) }}
                            </td>
                        </tr>
                    @endif
                @endforeach
                @foreach (collect($reservation['bills'][0]['breakdowns'])->where('unit_type', 3) as $key => $value)
                    @if (strpos($value['unit_item'], '割引料金') === false)
                        <tr>
                            ...
                            <td>
                                {{ Form::text('service_breakdown_subtotal[]', $value['unit_subtotal'], ['class' => 'form-control', 'readonly']) }}
                            </td>
                        </tr>
                    @endif
                @endforeach
...
<tr>
    ...
    <td colspan="1">
        <p class="text-left">合計</p>
        {{ Form::text('others_price', (int) $reservation['bills'][0]['others_price'], [
            'class' => 'form-control
                          col-xs-3',
            'readonly',
        ]) }}
...
<td>消費税：</td>
<td>
{{ Form::text('master_tax', floor(ReservationHelper::getTax((int) $reservation['bills'][0]['master_subtotal'] + (int) $discounts)), ['class' => 'form-control text-right', 'readonly']) }}
</td>
</tr>
<tr>
<td class="font-weight-bold">合計金額</td>
<td>
{{ Form::text('master_total', floor(ReservationHelper::taxAndPrice((int) $reservation['bills'][0]['master_subtotal'] + (int) $discounts)), ['class' => 'form-control text-right', 'readonly']) }}
</td>
</tr>
</table>
</div>
