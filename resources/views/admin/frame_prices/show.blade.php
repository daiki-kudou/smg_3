...
<section class="section-wrap bg-white wrap_shadow">
  ...
  <h3 class="d-block"><span
      class="mr-3">ID:{{ ReservationHelper::fixId($venue->id)}}</span>{{$venue->name_area}}・{{$venue->name_bldg}}{{$venue->name_venue}}
  </h3>
    ...
    <table class="table table-bordered table_fixed">
      ...
      <tbody>
        ...
        <tr>
          ...
          @if ($loop->first)
          <td rowspan="{{$frame_prices->count()}}">{{ number_format($frame_price->extend)}}円</td>
          @endif
        </tr>
        @endforeach
      </tbody>
