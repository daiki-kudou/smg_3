...
*/
  ...
  public function PreReservationStore($data, $status = 0)
  {
    $result = $this->create([
      ...
      'event_name1' => $data['event_name1']??$data['board_flag']?$data['event_name1'] : NULL,
      'event_name2' => $data['event_name2']??$data['board_flag']?$data['event_name2'] : NULL,
      'event_owner' => $data['event_owner']??$data['board_flag']?$data['event_owner'] : NULL,
  ...
  public function specificUpdate($request, $result, $venue_id)
  {
    ...
    DB::transaction(function () use ($request, $result, $venue_id, $splitKey) {
      ...
      $this->update([
        ...
        'event_name1' => !empty($request->{'event_name1_copied' . $splitKey})&&!empty($request->{'board_flag_copied'.$splitKey}) ? $request->{'event_name1_copied' . $splitKey}:null,
        'event_name2' => !empty($request->{'event_name2_copied' . $splitKey})&&!empty($request->{'board_flag_copied'.$splitKey}) ? $request->{'event_name2_copied' . $splitKey}:null,
        'event_owner' => !empty($request->{'event_owner' . $splitKey})&&!empty($request->{'board_flag_copied'.$splitKey}) ?$request->{'event_owner' . $splitKey}: NULL,
    ...
    if (!empty($data['search_free'])) {
      if (preg_match('/^[0-9!,]+$/', $data['search_free'])) {
        ...
        $searchTarget = $searchTarget->where(function ($query) use ($data) {
          if (!empty($data['search_free'])) {
            $query->whereRaw('pre_reservations.reserve_date = ? ', [$data['search_free']])
              ->orWhereRaw('pre_reservations.created_at between ? AND ?', [$data['search_free'] . ' 00:00:00', $data['search_free'] . ' 23:59:59']);
          }
        });
      } else {
        //文字列の場合
        $searchTarget = $searchTarget->where(function ($query) use ($data) {
          if (!empty($data['search_free'])) {
            $query->whereRaw('concat(venues.name_area,venues.name_bldg,venues.name_venue) LIKE ? ', ['%' . $data['search_free'] . '%'])
              ->orWhereRaw('users.company LIKE ?', ['%' . $data['search_free'] . '%'])
